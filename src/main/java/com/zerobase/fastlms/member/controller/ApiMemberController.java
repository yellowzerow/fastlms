...
public class ApiMemberController {
    ...
    public ResponseEntity<?> cancelCourse(
            Model model,
            @RequestBody TakeCourseInput input,
            Principal principal
    ) {
        String userId = principal.getName();

        //내 강좌인지 확인
        TakeCourseDto detail = takeCourseService.detail(input.getTakeCourseId());
        if (detail == null) {
        ...
        if (userId == null || !userId.equals(detail.getUserId())) {
            //내 수강신청 정보가 아닌 상황
            ResponseResult responseResult = new ResponseResult(
                    false, "본인의 수강신청만 취소할 수 있습니다.");
            return ResponseEntity.ok().body(responseResult);
        }

        ServiceResult result = takeCourseService.cancel(input.getTakeCourseId());
        if (!result.isResult()) {
